...
bool TestPipeLine::Draw(gc_ptr<YD3D::ResourcePackage>& package)
{
	package->State.set_state(EResourcePackageState::ERENDERING);
	return PostToCommandQueue(package);
}

bool TestPipeLine::PopulateCommandList(gc_ptr<YD3D::ResourcePackage> package, ID3D12GraphicsCommandList* commandList)
{
